...
if ($Timer.IsPastDue) {
    Write-Verbose "PowerShell timer is running late!"
}
	...
	function TryUpdateSessionHostDrainMode
    {
		...
		Process 
        {
			...
			if ($PSCmdlet.ShouldProcess($SessionHostName, "Update session host to set allow new sessions to $AllowNewSession"))
            {
				try 
				{ 
					$SessionHost = $VM.SessionHost = Update-AzWvdSessionHost -HostPoolName $HostPoolName -ResourceGroupName $ResourceGroupName -Name $SessionHostName -AllowNewSession:$AllowNewSession
